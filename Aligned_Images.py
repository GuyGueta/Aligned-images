    ...
    for angle in range(180):
        ...
        if 0 < angle < 90:
            for dist in range(distance):
                pixel_lst = Aligned_Images_Helper.pixels_on_line(image,
                                                      math.radians(angle),
                                                      dist, True)
                rank += rank_for_line(list_of_whites(pixel_lst, image))
                new_pixel_lst = Aligned_Images_Helper.pixels_on_line(image, math.radians(
                    angle), dist, False)
...
def photo_correction(image_source, output_name, max_diagonal):
    ...
    image = Aligned_Images_Helper.load_image(image_source)
    corrected_image = make_correction(image, max_diagonal)
    Aligned_Images_Helper.save(corrected_image, output_name)
